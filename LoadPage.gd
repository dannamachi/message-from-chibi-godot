...
func load_to_slot():
	if current_selected != -1:
		var results = cmd_runner.run_load_command(current_selected)
		if results[0]:
#			reload()
#			current_selected = -1
			hide()
			var newSectData = cmd_runner.api.get_section_data()
			emit_signal("loaded",newSectData)
		else:
			save_error()
			
func run_restart():
	var results = cmd_runner.run_restart_command()
	if results[0]:
		emit_signal("loaded",{})
	else:
		popup()
